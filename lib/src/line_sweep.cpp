...
void LineSweep::handleEventPoint(Event* e) {
    ...
    if (e->upper.size() + e->contain.size() == 0) {
        ...
        ComparableSegment* rightNeighbour = status.upper_bound(ComparableSegment(*e->pt, *e->pt, &sweep_line_y));
        if (rightNeighbour != status.end()->val and rightNeighbour != status.begin()->val) {
            // auto leftNeighbour = rightNeighbour--;  // TODO: Check if the predecssor exists?
            ComparableSegment* leftNeighbour = status.predecessor(*rightNeighbour)->val;
        ...
        ComparableSegment* leftNeighbour = status.upper_bound(leftmost);    // --leftNeighbour;
        ComparableSegment* rightNeighbour = status.upper_bound(rightmost);  // --rightNeighbour;
